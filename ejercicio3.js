...
piensa en la mejor forma de hacer un conteo de cada vez que ese sonido se repita como favorito en cada 
usuario.
...
for (const user of users) {
    for (const sound in user.favoritesSounds) {
        if(counterSounds[sound]) {
            counterSounds[sound]++;
        } else {
            counterSounds[sound] = 1;
        }
    }
}

console.log(counterSounds);